...
export default function CreatePost() {
    ...
    const generateImageFun = async () => {
        ...
        await GenerateAIImage({prompt: post.prompt}).then((res) => {
            setPost({...post, imgSrc : `data:image/jpeg;base64,${res?.data?.imgSrc}`});
    ...
    const createPostFun = async () => {
        ...
        await CreateNewPost(post).then((res) => {
    ...
    return(
        <section className="min-h-[800px] w-full text-white py-10">
            ...
            <button className="roboto inline-flex items-center justify-center whitespace-nowrap text-md font-medium focus-visible:outline-none disabled:pointer-events-none hover:bg-red-600 focus:outline-none focus:bg-red-600 focus:text-black disabled:opacity-50 h-10 bg-[#4b5563] text-white px-8 py-3 rounded-md flex-1"
            disabled = {post.prompt === ""}
            onClick={generateImageFun}
            >
            <AutoAwesomeRoundedIcon /><span className="ml-1">Generate Image</span>
            </button>
            <button 
            ...
            disabled = {post.prompt === "" || post.author === "" || post.imgSrc === ""}
            onClick={createPostFun}
            >
            <CreateRoundedIcon /><span className="ml-1">Post Image</span>
            </button>
            </div>
            </div>

            <GeneratedImage post={post} setPost={setPost} imageLoading={imageLoading} postLoading={postLoading} setImageLoading={setImageLoading} setPostLoading={setPostLoading} />
            </div>
        </section>
    );
